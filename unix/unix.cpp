...
uint32 S9xReadJoypad (int which1)
{
	...
	if(which1 == 0){
		...
		if (joy_buttons[which1][sfc_joy[UP_1]] || keyssnes[sfc_key[UP_1]] == SDL_PRESSED || joy_axes[which1][joyaxis_UD] == UP )	val |= SNES_UP_MASK;
		if (joy_buttons[which1][sfc_joy[DOWN_1]] || keyssnes[sfc_key[DOWN_1]] == SDL_PRESSED || joy_axes[which1][joyaxis_UD] == DOWN)	val |= SNES_DOWN_MASK;
		if (joy_buttons[which1][sfc_joy[LEFT_1]] || keyssnes[sfc_key[LEFT_1]] == SDL_PRESSED || joy_axes[which1][joyaxis_LR] == LEFT)	val |= SNES_LEFT_MASK;
		if (joy_buttons[which1][sfc_joy[RIGHT_1]] || keyssnes[sfc_key[RIGHT_1]] == SDL_PRESSED || joy_axes[which1][joyaxis_LR] == RIGHT)  val |= SNES_RIGHT_MASK;		
	}else{
		if (keyssnes[sfc_key[L_2]] == SDL_PRESSED || joy_buttons[which1][sfc_joy[L_2]])		val |= SNES_TL_MASK;
		if (keyssnes[sfc_key[R_2]] == SDL_PRESSED || joy_buttons[which1][sfc_joy[R_2]])		val |= SNES_TR_MASK;
		if (keyssnes[sfc_key[X_2]] == SDL_PRESSED || joy_buttons[which1][sfc_joy[X_2]])		val |= SNES_X_MASK;
		if (keyssnes[sfc_key[Y_2]] == SDL_PRESSED || joy_buttons[which1][sfc_joy[Y_2]])		val |= SNES_Y_MASK;
		if (keyssnes[sfc_key[B_2]] == SDL_PRESSED || joy_buttons[which1][sfc_joy[B_2]])		val |= SNES_B_MASK;
		if (keyssnes[sfc_key[A_2]] == SDL_PRESSED || joy_buttons[which1][sfc_joy[A_2]])		val |= SNES_A_MASK;
		if (keyssnes[sfc_key[START_2]] == SDL_PRESSED || joy_buttons[which1][sfc_joy[START_2]])	val |= SNES_START_MASK;
		if (keyssnes[sfc_key[SELECT_2]] == SDL_PRESSED || joy_buttons[which1][sfc_joy[SELECT_2]])	val |= SNES_SELECT_MASK;
		if (joy_buttons[which1][sfc_joy[UP_2]] || keyssnes[sfc_key[UP_2]] == SDL_PRESSED || joy_axes[which1][joyaxis_UD] == UP )	val |= SNES_UP_MASK;
		if (joy_buttons[which1][sfc_joy[DOWN_2]] || keyssnes[sfc_key[DOWN_2]] == SDL_PRESSED )	val |= SNES_DOWN_MASK;
